...
class BaseSinfonierDrain(storm.BasicBolt):
  ...
  def initialize(self, stormconf, context):
        ...
        excludedparams = ["drain","parallelism","sources","source","sourceId","grouping"]

        try:
            # Parse all XML file to tree variable
            tree = ET.parse(self.xmlPath)
            
            # Get all drain elements from tree
            for elt in tree.getiterator("drain"):
                # context["-1"] is drainId of this drain
                if context["task->component"][self.moduleid] == elt.get("abstractionId"):
                  # Get subelements in drain element
